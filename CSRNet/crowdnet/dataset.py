...
from crowdnet.datautils import load_gt, load_image,load_dataset_paths
from crowdnet.datautils import preprocess_transform, random_crop
from crowdnet.datautils import resize_img, generate_density_map
...
def get_train_transform(
    ...
    Args:
        ...
        input_w (int, optional):image width. Defaults to 512.

    Returns:
        Callable[[Image.Image, np.ndarray], Tuple[Tensor, np.ndarray]]: transform function
    """
    def transform(
        img: Image.Image,
...
def test_transform(img: Image.Image, density_map: np.ndarray) -> Tuple[Tensor, np.ndarray]:
    """ function to test single image and density map 
    ...
    Returns:
        Tuple[Tensor, np.ndarray]: tensor image and density map
    """
...
class JHUCrowdDataset(Dataset):
    """ PyTorch custom dataset class to load images and density maps of
        Jhucrowd dataset.
    """
    img_extensions = {".jpg"}
